...
```{r}
epi1_new_peaks <- read.table(file = "~/zbf_scATAC_v3/new_peaks_spdome/epi1_peaks_formatted.bed")
epi2_new_peaks <- read.table(file = "~/zbf_scATAC_v3/new_peaks_spdome/epi2_peaks_formatted.bed")
evl_new_peaks <- read.table(file = "~/zbf_scATAC_v3/new_peaks_spdome/evl_peaks_formatted.bed")
ysl_new_peaks <- read.table(file = "~/zbf_scATAC_v3/new_peaks_spdome/ysl_peaks_formatted.bed")

epi1_new_peaks <- StringToGRanges(epi1_new_peaks$V4)
epi2_new_peaks <- StringToGRanges(epi2_new_peaks$V4)
evl_new_peaks <- StringToGRanges(evl_new_peaks$V4)
ysl_new_peaks <- StringToGRanges(ysl_new_peaks$V4)

bl.peaks <- read.table(file = "~/zbf_scATAC_v3/new_peaks_spdome/aggregated_scATAC_peaks_formatted.bed")

bl.peaks <- StringToGRanges(bl.peaks$V4)

gr.epi1_qc <- epi1_new_peaks[is.na(findOverlaps(epi1_new_peaks, bl.peaks, select="arbitrary"))]
gr.epi2_qc <- epi2_new_peaks[is.na(findOverlaps(epi2_new_peaks, bl.peaks, select="arbitrary"))]
...
combine.peaks <- reduce(x = c(gr.epi1_qc , gr.epi2_qc , gr.evl_qc , gr.ysl_qc))
